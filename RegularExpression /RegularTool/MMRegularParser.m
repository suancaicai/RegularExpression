...
#pragma mark - 
#pragma mark - MMRegularNode

@interface MMRegularNode : NSObject

@property (assign, nonatomic) NSRange sourceRange;
@property (weak, nonatomic) MMRegularNode *parent;
@property (weak, nonatomic) MMRegularNode *nextSibling;
- (NSString*)displayString:(NSString*)pattern;
@end

@implementation MMRegularNode
- (NSString*)displayString:(NSString*)pattern
{
...
#pragma mark -
#pragma mark - MMRegularGroup

@interface MMRegularGroup : MMRegularNode
...
@interface MMRegularAlternation :MMRegularNode

@property (strong, nonatomic) NSArray *children;

@end

@implementation MMRegularAlternation
...
#pragma mark -
#pragma mark - MMRegularQuantifier

@interface MMRegularQuantifier :MMRegularNode

- (id)initWithFrom:(NSUInteger)from to:(NSUInteger)to;

@property (strong, nonatomic) MMRegularNode *child;
...
@implementation MMRegularQuantifier
...
#pragma mark -
#pragma mark - MMRegularCharacterBase

@interface MMRegularCharacterBase : MMRegularNode
...
@implementation MMRegularCharacterBase
...
#pragma mark -
#pragma mark -MMRegularCharacterSet


@interface  MMRegularCharacterSet :  MMRegularCharacterBase
...
@implementation MMRegularCharacterSet
...
#pragma mark -
#pragma mark - MMRegularLiteral

@interface MMRegularLiteral : MMRegularCharacterBase

@property (assign, nonatomic) unichar character;

@end

@implementation MMRegularLiteral
...
#pragma mark -
#pragma mark -MMRegularAnyCharacter


@interface MMRegularAnyCharacter : MMRegularCharacterBase

@end

@implementation MMRegularAnyCharacter
...
#pragma mark -
#pragma mark -MMRegularEndOfString


@interface MMRegularEndOfString : MMRegularCharacterBase

@end

@implementation MMRegularEndOfString
...
#pragma mark -
#pragma mark - MMRegularState

@interface MMRegularState : NSObject
{
...
#pragma mark -
#pragma mark -MMRegularTransition

@interface MMRegularTransition : NSObject

@property (weak, nonatomic) MMRegularCharacterBase *node;
@property (weak, nonatomic) MMRegularLiteral *bypassNode;
@property (strong, nonatomic) MMRegularState *nextState;

@end

@implementation MMRegularTransition
...
#pragma mark -
#pragma mark - MMRegularParser

@implementation MMRegularParser
...
- (NSString*)nodeDescription:(MMRegularNode*)node withLevel:(NSUInteger)level
{
    ...
    [buffer appendString:@"\n"];
    if ([node isKindOfClass:[MMRegularAlternation class]])
    {
        for (MMRegularNode* c in [(MMRegularAlternation*)node children])
            [buffer appendString:[self nodeDescription:c withLevel:level+1]];
    }
    else if ([node isKindOfClass:[MMRegularGroup class]])
    {
        for (MMRegularNode* c in [(MMRegularGroup*)node children])
            [buffer appendString:[self nodeDescription:c withLevel:level+1]];
    }
    else if ([node isKindOfClass:[MMRegularQuantifier class]])
    {
        [buffer appendString:[self nodeDescription:[(MMRegularQuantifier*)node child] withLevel:level+1]];
    }
...
- (MMRegularCharacterBase*)parseCharset:(NSString*)pattern inRange:(NSRange)range enclosed:(BOOL)enclosed
{
    BOOL negation = NO;
    NSUInteger count = 0;
    unichar lastChar = 0;
    ...
    if (!negation && count == 1) {
        MMRegularLiteral *l = [[MMRegularLiteral alloc] init];
        l.character = lastChar;
        l.sourceRange = range;
        l.ignoreCase = _ignoreCase;
        return l;
    } else {
        MMRegularCharacterSet *s = [[MMRegularCharacterSet alloc] init];
        s.negation = negation;
        s.chars = chars;
        s.sourceRange = range;
...
- (MMRegularGroup*)groupFromNodes:(NSArray*)nodes enclosed:(BOOL)enclosed
{
    if (nodes.count == 1) {
        MMRegularGroup *t = [nodes objectAtIndex:0];
        
        if ([t isKindOfClass:[MMRegularGroup class]]) {
            t.capturing |= enclosed;
            return t;
        }
    }

    MMRegularGroup *g = [[MMRegularGroup alloc] init];
    g.children = [nodes copy];
    g.capturing = enclosed;

    // setup links
    MMRegularNode *prev = [g.children objectAtIndex:0];
    prev.parent = g;

    for (NSUInteger i = 1; i < g.children.count; i++) {

        MMRegularNode *curr = [g.children objectAtIndex:i];
        curr.parent = g;
...
- (MMRegularGroup*)parseSubpattern:(NSString*)pattern inRange:(NSRange)range enclosed:(BOOL)enclosed
{
    ...
    for (NSUInteger i = 0; i < range.length; i++) {
        ...
        if (escape) {
            ...
            if (lastnode == nil || [lastnode isKindOfClass:[MMRegularQuantifier class]])
            ...
            for (NSUInteger j = i + 1; j < range.length; j++) {
                ...
                if (d == '}') {
                    ...
                    MMRegularQuantifier *qtf = [[MMRegularQuantifier alloc] init];
            ...
            [nodes removeLastObject];
            MMRegularQuantifier *qtf = [[MMRegularQuantifier alloc] initWithFrom:0 to:NSUIntegerMax];
            qtf.child = lastnode;
            lastnode.parent = qtf;
            lastnode = qtf;
            [nodes addObject:lastnode];
            ...
            [nodes removeLastObject];
            MMRegularQuantifier *qtf = [[MMRegularQuantifier alloc] initWithFrom:1 to:NSUIntegerMax];
            qtf.child = lastnode;
            lastnode.parent = qtf;
            lastnode = qtf;
            [nodes addObject:lastnode];
            ...
            if ([lastnode isKindOfClass:[MMRegularQuantifier class]]) {
                // greediness modifier
                [(MMRegularQuantifier*)lastnode setGreedy:NO];
            } else {
                [nodes removeLastObject];
                MMRegularQuantifier *qtf = [[MMRegularQuantifier alloc] initWithFrom:0 to:1];
                qtf.child = lastnode;
                lastnode.parent = qtf;
                lastnode = qtf;
                [nodes addObject:lastnode];
            ...
            // any character
            lastnode = [[MMRegularAnyCharacter alloc] init];
            [nodes addObject:lastnode];
        }
        else if (c == '|') {
            // alternation
            ...
            MMRegularGroup *g = [self groupFromNodes:nodes enclosed:enclosed];
            ...
            lastnode = [[MMRegularEndOfString alloc] init];
            [nodes addObject:lastnode];
    ...
    if (alternations != nil) {
        ...
        MMRegularAlternation *a = [[MMRegularAlternation alloc] init];
        a.children = alternations;
        a.sourceRange = NSMakeRange(range.location, endPos);

        // setup links
        MMRegularNode *prev = [alternations objectAtIndex:0];
        prev.parent = a;

        for (NSUInteger i = 1; i < alternations.count; i++) {
            MMRegularNode *curr = [alternations objectAtIndex:i];
            curr.parent = a;
            prev.nextSibling = curr;
            prev = curr;
        }

        g = [[MMRegularGroup alloc] init];
...
- (NSString*)reformatString:(NSString *)input
{
    ...
    @autoreleasepool {
        MMRegularState *initialState = [[MMRegularState alloc] init];
        MMRegularState *finalState = [self processNode:_node fromState:initialState length:input.length];

       MMRegularState *x = [self nextState:initialState finalState:finalState input:tInput position:0];

        return x.isFinal ? tInput : nil;
    }
}

- (MMRegularState*)processNode:(MMRegularNode*)node fromState:(MMRegularState*)state length:(NSUInteger)length
{
    if ([node isKindOfClass:[MMRegularEndOfString class]])
    {
        MMRegularState *finalState = [[MMRegularState alloc] init];
        finalState.isFinal = YES;

        MMRegularTransition *tran = [[MMRegularTransition alloc] init];
        tran.node = (MMRegularCharacterBase*)node;
        tran.nextState = finalState;
        [state.transitions addObject:tran];

        return finalState;
    }
    else if ([node isKindOfClass:[MMRegularCharacterBase class]])
    {
        MMRegularState *finalState = [[MMRegularState alloc] init];
    ...
    {
        MMRegularQuantifier *qtf = (MMRegularQuantifier*)node;

        MMRegularState *curState = state;
        ...
        for (NSUInteger i = qtf.countFrom; i < MIN(qtf.countTo, length); i++) {
            MMRegularState *nextState = [self processNode:qtf.child fromState:curState length:length];
    ...
    {
       MMRegularGroup *grp = (MMRegularGroup*)node;
        ...
        if (!grp.capturing && grp.children.count == 1 && [[grp.children objectAtIndex:0] isKindOfClass:[MMRegularLiteral class]])
        {
            MMRegularTransition *tran = [[MMRegularTransition alloc] init];
            tran.node = nil;
            tran.bypassNode = (MMRegularLiteral*)[grp.children objectAtIndex:0];
    ...
    else if ([node isKindOfClass:[MMRegularAlternation class]])
    {
        MMRegularAlternation *alt = (MMRegularAlternation*)node;

        MMRegularState *finalState = [[MMRegularState alloc] init];

        for (NSUInteger i = 0; i < alt.children.count; i++) {
            MMRegularState *curState = [self processNode:[alt.children objectAtIndex:i] fromState:state length:length];
...
- (MMRegularState*)nextState:(MMRegularState*)state finalState:(MMRegularState*)final input:(NSMutableString*)input position:(NSUInteger)pos
{
    if (state.isFinal) return state;

    if (pos > input.length) return final;

    for (MMRegularTransition *tran in state.transitions) {
        ...
        MMRegularState *s = [self nextState:tran.nextState finalState:final input:input position:nextPos];
        if (s.isFinal)
        {
